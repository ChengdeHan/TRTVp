...
img=double(imread('terrasar_city_512.jpg'));
img=img(:,:,1);
[m, n] = size(img);
y=max(img,1);
f= double(y)/255;
...
%% TVp_AA
inputPara.p=0.4;              %   0<p<1
inputPara.destroyedimage=f;
inputPara.lamda=11;    %  If lamda is too large, the noise cannot be removed cleanly; and if lamda is too small, the fine features will be oversmoothed.
inputPara.r_t=10;  
inputPara.r_w=10;   
...
imshow(img_ratio);
title('ratio img');
imwrite(img_ratio,'ratio_city_TRTVp.png');
...
hold off
I=u(start_row:end_row, start_col:start_col );
av=mean2(I);
st=std2(I);
enl=av^2/st^2
